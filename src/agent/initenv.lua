...
function torchSetup(_opt)
    ...
    if opt.agent_params then
        ...
        if opt.image_compare_network ~= '' then
            opt.agent_params.image_compare_network = opt.image_compare_network
        end
    end
...
function setup(_opt)
    ...
    aleWrap = dqn.ALEWrapper(_opt)
    local gameActions = aleWrap:getLegalActions()

    --- first things first
    local opt = torchSetup(_opt)

    -- agent options
    _opt.agent_params.useRGB                    = _opt.env_params.useRGB
    _opt.agent_params.repeat_action_probability = _opt.env_params.repeat_action_probability
    ...
    if _opt.image_compare_network ~= '' then
        _opt.agent_params.image_compare_network = _opt.image_compare_network
    end
